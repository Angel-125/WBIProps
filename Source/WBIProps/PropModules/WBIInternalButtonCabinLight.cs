...
namespace WBIProps
{
    ...
    public class WBIInternalButtonCabinLight : InternalModule
    {
        ...
        protected ModuleColorChanger lightModule;

        public void Start()
        {
            lightModule = this.part.FindModuleImplementing<ModuleColorChanger>();
            if (lightModule != null && lightModule.toggleInFlight)
            {
                if (lightModule.Events["ToggleEvent"].guiName == lightModule.eventOnName)
                    lightState = ELightStates.On;
                else if (lightModule.Events["ToggleEvent"].guiName == lightModule.eventOffName)
        ...
        public override void OnUpdate()
        {
            ...
            if (lightModule != null)
            {
                if (lightModule.Events["ToggleEvent"].guiName == lightModule.eventOnName && lightState == ELightStates.Off)
                    SetupLights(true);
                else if (lightModule.Events["ToggleEvent"].guiName == lightModule.eventOnName && lightState != ELightStates.Off)
                    SetupLights(true);
            }
        }
